...
class SharedState:
    displays_hit_boxes: bool
    game_duration: float
    enemy_to_ally_ratio: tuple
...
SharedStuff: TypeAlias = tuple[SharedData, SharedState]
'''
This is passed to each scene as the ``userdata`` argument.
'''


async def run(parent: FloatLayout, *, displays_hit_boxes=False, game_duration=30.0, enemy_to_ally_ratio=(8, 2)):
    '''
    :param parent: The widget in which the game runs.
    :param game_duration: The duration of each game session.
    :param enemy_to_ally_ratio: The ratio of enemies to allies spawned in the game.
    '''
    ...
    first_scene = "game" if "HOMOLE_SKIP_TITLE_SCENE" in os.environ else "title"
    await sceneswitcher.run(
        f"whack_a_homole.scenes.{first_scene}.main",
        transition.fade,
        parent=parent,
        userdata=(
            SharedData(
            ...
            SharedState(
                displays_hit_boxes=displays_hit_boxes,
                game_duration=game_duration,
                enemy_to_ally_ratio=enemy_to_ally_ratio,
            ),
        ),
    )
