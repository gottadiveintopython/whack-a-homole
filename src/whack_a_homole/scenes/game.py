...
from kivy.properties import (
    NumericProperty, BoundedNumericProperty, ObjectProperty, ColorProperty, BooleanProperty,
)
...
from whack_a_homole import SharedStuff, uix
from whack_a_homole.utils import is_colliding_and_not_wheel, show_fading_image, whiteout_transition
...
<PartiallyRevealableImage>:
    ...
    canvas:
        ...
        Color:
            rgba: 1, 0, 0, (1 if self.shows_bbox else 0)
        Line:
            width: 2
...
class PartiallyRevealableImage(Widget):
    shows_bbox = BooleanProperty(False)
...
async def main(parent: FloatLayout, userdata: SharedStuff, *, _cache=[]):
    ...
    with ExitStack() as stack:
        ...
        def reflect_score_to_label(_1, _2, label=root.ids.score_label, state=state):
            label.text = str(state.score) + "pts"
        stack.enter_context(pyle.immediate_rule(reflect_score_to_label, trigger_callback_on_activate=True))
...
async def play_hurt_effect_on(target: Widget):
    draw_target = target.canvas.after
    with draw_target:
        color = Color(1, 0, 0, 0.3)
        rect = Rectangle(size=target.size, pos=target.pos)
    try:
